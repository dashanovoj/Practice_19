...
public class Task2 {
    ...
    // Создаём пользовательское исключение для случая, когда студент не найден
    ...
    // Создаём метод, который ищет студента по имени и выбрасывает исключение при ненахождении такового
    public static Student findStudentByName(List <Student> students, String name) throws StudentNotFoundException {
        for (Student student : students) {
            if (student.getName().equalsIgnoreCase(name)) { // через геттер и метод, игнорирующий регистр, сравниваем имена
                return student;
            }
        }
        throw new StudentNotFoundException("Student named " + name + " is not found");
    }
    ...
    public static class QuickSort {
        ...
        private static void swap(List<Student> students, int i, int j) {
            ...
            students.set(i, students.get(j)); // метод set заменяет элемент в массиве с указанием индекса и нового значения
            students.set(j, temp);
        }
    }
    ...
    public static void main(String[] args) {
        ...
        // Поиск студента
        System.out.println("Enter the student's name to be found: ");
        String searchName = sc.next();

        // Используем блок try-catch для выполнения кода и обработки исключения
        try {
            Student foundStudent = findStudentByName(students, searchName);
            System.out.println("The found student: " + foundStudent);
